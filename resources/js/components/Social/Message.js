...
import { GetAllMsg,saveMsg ,AllFriendData} from '../Services/Admin/AdminServices';
...
import Echo from 'laravel-echo';
 class Message extends Component {
    ...
    //     console.log('msg',this.state.msg);
        window.Echo.private(`messages.${this.state.friendid}`)
        .listen ('NewMessage',(e)=>{
     ...
     chatConversation=async(friendId)=>{
         ...
         this.setState({ chatStart:true ,friendid:friendId ,name:name,selectUser:true});
         const getLoginData = localStorage.getItem("LoginData");
         const data1 = JSON.parse(getLoginData);
         const myid = data1.user.id;
      ...
      if(response.success){
          ...
          this.setState({ allMsg:response.data  },
            this. scrollToMyRef()
            );
      }
    ...
    //  }
     onchangeInput=(e)=>{
         this.setState({ msg:e.target.value,Submitbutton:!this.state.Submitbutton  });
     }
     MsgEvent=async(e)=>{
         e.preventDefault();
           ...
           <div class="topMargin">
               <div class="row">
                   ...
                   <div class="messageSidebar">
                       ...
                       <div class="msgfriend" id={this.state.friendid==row.id?'activeUser':'inactiveUser'}>
                           <img  src={`${PUBLIC_URL}storage/app/public/uploads/mbstu2.png`}/>
                           <h5 onClick={()=>this.chatConversation(row.id)}>{row.name}</h5>
                           <span>12</span>
                       </div>
                           ))}
                       </div>
                   ...
                   <div class="messageRightbar">
                       <div class="head">
                           <div class="row">
                               <div class="col-md-9">
                                   <div class="subhead">
                               <img  src={`${PUBLIC_URL}storage/app/public/uploads/mbstu2.png`}/>
                               <h6 style={{paddingTop:'10px',paddingLeft:'10px'}}>{this.state.name}</h6>
                               </div>
                               </div>
                               <div class="col-md-3">
                               <button class="btn" onClick={()=>this.VedioFun()}><i class="fa fa-video-camera"></i></button>
                    ...
                    {this.state.errors && this.state.errors.msg && (
                                <p class="text-danger" style={{fontSize:'10px',
                                    marginBottom:'-7px'}}>{this.state.errors.msg[0]}</p>
                            )}
